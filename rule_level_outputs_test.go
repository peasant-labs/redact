...
func TestRedactText_PanicRecoveryUsesMaximumRulePrecedence(t *testing.T) {
	...
	for i := range fixtures {
		...
		if recoveryFixture != nil {
			t.Fatalf("pkg/redact/testdata/rule_level_outputs.yaml defines more than one panicFallbackOutput; keep one load-bearing recovery case so the induced panic is unambiguous")
		}
		recoveryFixture = &fixtures[i]
	}
	if recoveryFixture == nil {
		t.Fatal("pkg/redact/testdata/rule_level_outputs.yaml has no panicFallbackOutput; add one case that proves panic recovery applies Maximum-only rules before broader matches")
	}
...
func loadRuleLevelOutputFixtures() ([]ruleLevelOutputFixture, error) {
	...
	if err := yaml.Unmarshal(ruleLevelOutputsData, &fixtures); err != nil {
		return nil, fmt.Errorf("redact: failed to parse pkg/redact/testdata/rule_level_outputs.yaml while loading rule-level output fixtures: %w; the activation and output tests cannot run; fix the YAML syntax", err)
	}
	expectedCount := len(projectRules) + 4
	if len(fixtures.Cases) != expectedCount {
		return nil, fmt.Errorf("redact: pkg/redact/testdata/rule_level_outputs.yaml loaded %d cases, want %d; every compiled project rule and all four SSH/email overlap controls must remain covered exactly once", len(fixtures.Cases), expectedCount)
	}
	...
	for i, fixture := range fixtures.Cases {
		if fixture.Name == "" || fixture.RuleID == "" || fixture.Input == "" {
			return nil, fmt.Errorf("redact: incomplete fixture at cases[%d] in pkg/redact/testdata/rule_level_outputs.yaml while loading rule-level output fixtures: name, ruleId, and input must be non-empty; the activation and output tests cannot identify or exercise this case; fill in the missing field", i)
		}
		if !fixture.Category.IsValid() {
			return nil, fmt.Errorf("redact: invalid category %q in pkg/redact/testdata/rule_level_outputs.yaml case %q while loading rule-level output fixtures: expected secrets, pii, paths, or project; the test cannot verify semantic classification; fix the fixture category", fixture.Category, fixture.Name)
		}
		if fixture.MinimumLevel != "" && !fixture.MinimumLevel.IsValid() {
			return nil, fmt.Errorf("redact: invalid minimumLevel %q in pkg/redact/testdata/rule_level_outputs.yaml case %q while loading rule-level output fixtures: expected minimal, standard, maximum, or empty for the category default; the test cannot verify activation policy; fix the fixture minimumLevel", fixture.MinimumLevel, fixture.Name)
		}
		if _, duplicate := seenNames[fixture.Name]; duplicate {
			return nil, fmt.Errorf("redact: fixture name %q is duplicated in pkg/redact/testdata/rule_level_outputs.yaml; subtest identity is ambiguous; give every case a unique name", fixture.Name)
		}
		...
		if _, isProjectRule := projectIDs[fixture.RuleID]; isProjectRule {
			...
			if _, duplicate := seenProjectRules[fixture.RuleID]; duplicate {
				return nil, fmt.Errorf("redact: project rule %q appears more than once in pkg/redact/testdata/rule_level_outputs.yaml; exact registry membership is ambiguous; keep one level-output case per project rule", fixture.RuleID)
			}
	...
	for ruleID := range projectIDs {
		if _, ok := seenProjectRules[ruleID]; !ok {
			return nil, fmt.Errorf("redact: pkg/redact/testdata/rule_level_outputs.yaml is missing compiled project rule %q; exact rule-level membership is incomplete; add its activation and output case", ruleID)
		}
	}
	for _, control := range []ruleLevelControl{ordinaryEmailControl, gitEmailWithoutPathControl, gitEmailFollowedByProseControl, unsupportedSCPPrefixControl} {
		if _, ok := seenControls[control]; !ok {
			return nil, fmt.Errorf("redact: pkg/redact/testdata/rule_level_outputs.yaml is missing email overlap control %q; exact control membership is incomplete; restore the case", control)
		}
	}
...
func TestRuleLevelOutputs(t *testing.T) {
	...
	for _, fixture := range fixtures {
		...
		t.Run(fixture.Name, func(t *testing.T) {
			...
			if rule == nil {
				t.Fatalf("fixture ruleId %q does not identify a compiled rule; fix pkg/redact/testdata/rule_level_outputs.yaml or restore the rule", fixture.RuleID)
			}
