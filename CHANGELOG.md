...
### Added
- `LICENSE`: the module now carries the standard Apache-2.0 text (`Copyright 2026 Peasant Labs`, byte-identical to the `peasant` module). Earlier versions shipped no license grant, so the code was all-rights-reserved by default. No rule or code changes; `RuleSetVersion` is unchanged at `3.1.1`.
